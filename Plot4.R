...
Plot4 <-function(){
    ...
    #Plot in row1 column1
    with(power_consumption_subset, plot(FullTimeDate, Global_active_power, type="l", xlab="Day", ylab="Global Active Power(Kilowatts)"))

    #plot in row1 column 2
    with(power_consumption_subset, plot(FullTimeDate, Voltage, type = "l", xlab="datetime", ylab="Voltage"))

    #Plot in row2 column 1
    ...
    #Plot in row2 column2
    with(power_consumption_subset, plot(FullTimeDate, Global_reactive_power, type="l", xlab="datetime", ylab="Global_reactive_power"))
    dev.off()
